...
| 4 | chokidar must be `^3.6.0` (see the pin section in CLAUDE.md) | **v5** is ESM-only → `ERR_REQUIRE_ESM` → Core never binds :3200/:3100 (v4 is dual-format but unvalidated here) | the repo + its `npm pack` tgz carry the pin (safe). Only `npm install -g lm-assist@latest` from the registry re-breaks it. |
...
| 7 | **`node_upgrade` with no `node` hits the hub's DEFAULT node** — the most-recently-connected one, chosen at the hub (`assist-api /internal/mcp-relay`), which this repo cannot change. Measured 2026-07-28: the default resolved to the Windows box while the caller was working on 117. | An upgrade replaces the build on a machine you never named, and the `⟦…⟧` provenance footer only tells you *after* its services have restarted. | `node_upgrade` now **refuses** a call with no `node`, and refuses one that lands on a node other than the one named (hub fallback, or the node belongs to the other connector's fleet). Always pass `node=<hostId \| hostname from list_nodes>`. It also echoes hub/cluster/node/current→intended build *before* dispatching. |
| 8 | A local `.tgz` `source` that does not exist on the target is **destructive, not inert** — `upgrade.js` `resolveSource()` falls through to an npm spec, and `main()` kills every service (step 2) *before* installing (step 3). | The node is left with Core/Web down and no new build, while the tool already answered `Upgrade started` (the route spawns detached with `stdio:'ignore'` and cannot see the child die). | `node_upgrade` now checks the path exists **on the target** before dispatching. `source` is a path on the machine being upgraded, not the one you are calling from — build it there with `./core.sh pack` or copy it across. URLs and `github:` specs are not disk-checked. |

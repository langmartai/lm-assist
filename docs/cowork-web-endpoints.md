...
| File attach → delivery | ✅ **cloud** (CRACKED 2026-07-13) / ✅ **local** | **CLOUD — fully cracked, headless (no browser):** two legs — (1) UPLOAD `POST /api/organizations/{org}/cowork/attachments` (multipart `file`, claude.ai cookie + the FULL browser fingerprint via `buildBrowserHeaders`, **minus `content-type`** so fetch sets the boundary; a bare-cookie/no-UA POST trips a **Cloudflare 403**) → `{file_uuid, file_name (SERVER-SANITIZED), size_bytes}`; (2) SEND `POST /v1/code/sessions/{cse}/events` (OAuth) with `file_attachments:[{file_name,file_uuid,is_image}]` **and** a `<uploaded_files>\n<file><file_path>{name}</file_path><file_uuid>{uuid}</file_uuid></file>\n</uploaded_files>\n\n{prompt}` content prefix. Runtime stages the file at **`/root/.claude/uploads/{session}/{uuid}-{name}`** (NOT `/mnt/user-data/uploads/` — earlier note was stale) and the agent reads it. **Verified live** (agent echoed the marker). Implemented in lm-assist: `core/src/cowork/cowork-attachments.ts` + `POST /cowork/attachments`. DEAD-ENDS: the OAuth Files API (`/v1/files`) uploads fine but its `file_id` does NOT resolve in cowork (a `document` content block is stripped; the same id in the native envelope → agent NOFILE) — the runtime resolves `file_uuid` ONLY from the `/cowork/attachments` store; and the generic `/api/{org}/upload`+`file_attachments` ref → agent NO-ACCESS. **LOCAL:** a file placed in the device ditto **`<ditto>/uploads/`** is bound into the sandbox at `../uploads/` (the `device_stage_files` desktop path). **Connectors for LOCAL** load from the ditto's `remoteMcpServersConfig` (account-wide), not per-session `mcp_connector_ids`. |

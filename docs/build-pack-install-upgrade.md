...
Graceful alt for the restart: **`node_lifecycle` / `POST /lifecycle/restart`** (self-respawn, no force-kill; the respawned Core appends to `core-{prod|dev}.log` and rewrites the pidfile). Verify each node post-deploy by a unique marker in the running dist (e.g. `grep canonicalByWorktree …/session-footprint-collector.js`) + `/health`.
...
- **Windows Core watchdog** (in the elevated worker, `LmAssistElevatedWorker`): restarts a Core that DIED — `core-prod.pid` present + `:3100` closed + that pid dead for 3 probes (~90 s) → `schtasks /run /tn LmAssistCoreInteractive` (10 min cooldown). ONLY through that task — its principal sets Core's session AND integrity; the worker is elevated, so without the task it refuses to restart rather than start an elevated Core (`watchdog.lastStart` says so). No pidfile = stopped on purpose (`lm-assist stop`, the upgrader and `node_lifecycle exit` remove it) → left alone. Off: `LM_CORE_WATCHDOG=0` or create `<data>/elevated/watchdog.off`. State: worker `GET 127.0.0.1:3110/health` → `watchdog`. If the worker task runs a COPIED runtime (107: `~/.lm-assist/worker-runtime`), a worker change must be copied there too (`elevated/*.js` + `utils/core-runtime-files.js`), then `schtasks /end` + `/run` the task.

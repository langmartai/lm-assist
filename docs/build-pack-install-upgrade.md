...
3. **`npm install -g <spec>`** — Windows EBUSY fallback `upgradeViaTarball()`: use the local `.tgz` (or `npm pack <spec>`), extract with `C:\Windows\System32\tar.exe` (git-tar misreads `C:` as a host), `robocopy /E /XD node_modules` in place (no rename), `npm install --omit=dev --ignore-scripts`.

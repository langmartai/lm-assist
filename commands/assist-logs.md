...
```bash
curl -s -H "x-api-key: $(cat "${LM_ASSIST_DATA_DIR:-$HOME/.lm-assist}/api-token" 2>/dev/null)" "http://localhost:3100/assist-resources/log?file=context-inject-hook.log&limit=${LIMIT}&search=${SEARCH}"
```

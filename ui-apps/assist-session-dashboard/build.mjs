#!/usr/bin/env node
/* Build the assist-session-dashboard pane: the ORIGINAL web session-dashboard page, bundled.
 *
 ...
 * next/navigation, the api-client probes) and the ConsoleTab stub.
 *
...
const sources = [
  path.join(webSrc, 'components', 'session-dashboard'),
  path.join(webSrc, 'app', '(dashboard)', 'session-dashboard'),
  path.join(webSrc, 'components', 'shared'),
  src,
].map((p) => `@source "${p}";`).join('\n');

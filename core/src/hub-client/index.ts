  ...
  private setupEventHandlers(): void {
    ...
    this.wsClient.on('authenticated', (data: { gatewayId: string; sessionId: string }) => {
      ...
      if (this.wsClient) {
        ...
        // Direct-TCP-for-LAN (native-speed bulk path). Rides the Core API port
        // via an HTTP Upgrade (no dedicated listener port) — we register the
        // receiver wiring and advertise our Core port to peers over the fabric
        // HELLO, so same-LAN transfers use kernel TCP instead of ~9 MB/s
        // userspace UDP.
        if (!this.tcpListenerStarted) {
          this.tcpListenerStarted = true;
          try {
            const { setTcpUpgradeDeps } = require('../transport/tcp-upgrade') as typeof import('../transport/tcp-upgrade');
            ...
            const port = this.options.localApiPort; // the already-open, LAN-reachable Core port
            setTcpUpgradeDeps({
            ...
            // a link that came up in the boot race before the endpoint was set
            // (setFabricSelfTcp re-runs readvertiseAll).
            const t = setTimeout(() => setFabricSelfTcp({ host: nodeIp, port }), 4000);
            if (t.unref) t.unref();
          } catch (e) {
            console.debug('[HubClient] TCP-for-LAN wiring failed (relay/UDP still work):', (e as Error).message);
          }
        }
      }

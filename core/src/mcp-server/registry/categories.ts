/** The tool → CATEGORY map, split out of catalog.ts so it can be imported from the
 *  tools/list path.
 *
 *  catalog.ts cannot be: it pulls every handler module (for the read-only
 *  Implementation view's `String(handler)`) and imports `../configure`, and its own
 *  header says configure.ts/stdio must NOT import it. But the profile filter that
 *  decides WHICH tools are advertised runs inside the tools/list closure in
 *  configure.ts and needs exactly this map — categories are how a profile names a
 *  group of tools.
 *
 *  So this module holds the DATA and nothing else: no handler imports, no configure
 *  import, no side effects. catalog.ts re-exports from here, so its public API and the
 *  completeness test that keeps this map exhaustive are unchanged.
 */
...
export const CATEGORY_ORDER: string[] = [
  ...
  'github', 'whatsapp', 'linkedin', 'gmail', 'desktop', 'backup', 'elevated', 'vm', 'container', 'ui',
];

/** Source dir prefix for the module paths recorded above. */
export const T = 'core/src/mcp-server/tools';
...
mod('expanded.ts', 'core', ['mcp_profile']);
...
mod('container.ts', 'container', [
  'container_status', 'container_run', 'container_power', 'container_logs', 'container_delete',
]);
// --- pluggable UI pages ---
mod('ui-pages.ts', 'ui', [
...
/** Category for an advertised tool, or null when it has no entry (the completeness
 *  test in mcp-tool-catalog.test.ts turns any such gap into a failure, so a null here
 *  means "new tool, not yet mapped" rather than "uncategorised by design"). */
export function categoryOf(name: string): string | null {
  return META[name]?.category ?? null;
}

/** Every tool name in a category. */
export function toolsInCategory(category: string): string[] {
  return Object.keys(META).filter((n) => META[n].category === category);
}

/** The raw map, for catalog.ts. */
export function categoryMeta(): Readonly<Record<string, Meta>> {
  return META;
}

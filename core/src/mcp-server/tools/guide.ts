...
🔴 DRIVING A TUI MENU/MODAL: ONE KEY PER CALL, VERIFYING BETWEEN. A combined \`terminal_send keys:["Down","Enter"]\` MIS-LANDED on a Claude Code modal — it confirmed the DEFAULT option 1 instead of the intended 2, silently flipping a live prod session's permission mode (measured 2026-08-15, node 117). Correct pattern: send the arrow → \`terminal_capture\` to confirm the \`❯\` ACTUALLY MOVED → only then send Enter. Never assume a keystroke landed; a modal that never sees its answer leaves the session frozen, and a modal that sees the WRONG answer changes real config. Model-switch and permission prompts have cost/security consequences — surface those to the human rather than answering them.
A session sitting on a modal is FROZEN, not working: it accepts nothing typed at it and any busy check reads it as mid-turn. \`terminal_capture\` twice — a byte-identical screen means frozen. Restarting one: guide("ccr").
...
RESTART A STUCK SESSION — \`ccr_restart\` RETURNS THE SCREEN; READ IT (measured 2026-08-15, node 117 prod)
\`ccr_restart(session_id)\` kills the owner and resumes it fresh (same id, same history, fresh MCP tools). Every result carries \`screen\` — the session's visible pane, plus \`tmuxSession\` — because the two states that matter are indistinguishable any other way. THE WAITING HAPPENS AFTER THE RESTART, NOT BEFORE IT: the call returns once the fresh TUI has SETTLED (it paints, loads the transcript, then opens its dialogs), so \`screen\` is the state it came up on and not a frame mid-redraw. \`screenStable:false\` = still changing when that window expired → re-read with \`terminal_capture\` rather than acting on that snapshot.
• 🔴 A SESSION FROZEN ON A MODAL READS AS "ACTIVELY BUSY". A model-switch confirm, a resume-from-summary prompt, or a new CLI version's first-run permission prompt all look exactly like a running turn to any detector, and the session is frozen INDEFINITELY, so waiting can never resolve. That is why restart REFUSES IMMEDIATELY (CONFLICT, \`busy:true\`) instead of waiting: it hands you the pane and nothing is killed. Read \`screen\`: a blocking dialog with \`screenStable:true\` (the pane stopped changing) means there is no in-flight turn to lose → answer it with \`terminal_send\`, or call again with \`force:true\`. Only pass \`wait_ms\` when you have SEEN real work in flight and want to wait for it.
• 🔴 "restarted" MEANS A PROCESS SPAWNED, NOT THAT THE SESSION IS USABLE — and a RESTART RE-OPENS MODALS, it does not clear them. A large session comes back on \`1. Resume from summary / 2. Resume full as-is / 3. Don't ask again\`; on a just-upgraded CLI you additionally get that version's first-run prompts. Expect a CHAIN of dialogs after every restart and budget for driving them. The returned \`screen\` is what it is showing right now — check it before assuming the session can take a turn.
• 🔴 DRIVE MENUS ONE KEY PER CALL, VERIFYING BETWEEN — see guide("terminals"). Nothing in lm-assist auto-dismisses these, deliberately: which modal is safe to answer is YOUR judgement, not a restart primitive's.
• A \`ccr_restart\` you gave a long \`wait_ms\` can still outlive the MCP connector's own timeout — the caller then sees \`The connector's server isn't responding\`, which reads like a dead node and IS NOT ONE. Nothing was killed, state is unchanged, the retry is safe: read tools keep answering and \`node_status services\` stays ok throughout. Re-call with the default (no wait) to get the real verdict plus the screen, and prove host liveness off this connector before suspecting the box — guide("access-paths").
• 🔴 A WEDGE THAT LOOKS LIKE A QUOTA LIMIT IS OFTEN A STALE CLI. \`Session model claude-opus-5 could not be restored (not a model this version of Claude Code recognizes) — using fable instead\`: an unrecognised model does NOT degrade to the newest one the CLI knows, it lands on FABLE, which has its own weekly window and is the model most likely to be at 100%. So version drift presents as \`You've reached your Fable limit\` with no visible version error. \`claude --version\` reports only what is on PATH and HIDES a second install (117 ran a native \`~/.local/bin\` copy shadowing an npm-global one) — cross-check \`which claude\`, \`npm ls -g --depth=0 | grep claude\`, \`npm view @anthropic-ai/claude-code version\`. Fix with \`claude update\`, and UPGRADE BEFORE RESTARTING (a running session keeps its old binary — otherwise you pay two restarts and two rounds of resume dialogs). Then PIN the model in \`/model\` rather than taking \`Default\`, which is what drifted.
...
const ALIASES: Record<string, string> = {
  ...
  // A wedged session is looked up in the words of the SYMPTOM — nobody staring at a
  // frozen modal or a "Fable limit" banner thinks to type "ccr".
  restart: 'ccr', stuck: 'ccr', wedged: 'ccr', frozen: 'ccr', modal: 'ccr', dialog: 'ccr', 'model-limit': 'ccr', fable: 'ccr', 'version-drift': 'ccr',

...
2. For each ACTIVE mission:
   ...
   e2. PENDING QUESTION: if \`mission_session_read(sid)\` returns a \`pendingQuestion\` (the executor is BLOCKED on an interactive AskUserQuestion — common when it could NOT reach the hub for a \`decide_gate\` agree-gate, e.g. an egress-403 blocked its lm-assist install), ANSWER it with \`mission_session_answer(sid, answer)\` (the answer = an option LABEL or free text). Do NOT use \`mission_session_drive\` for this — a plain prompt QUEUES BEHIND the blocking question instead of resolving it. This is distinct from a hub \`decide_gate\`. AFTER answering, re-read: if the executor stays idle/not-alive and never consumes the answer (cloud idle-suspend), \`mission_session_drive\` a wake nudge; if it still won't resume, report honestly (do NOT mark \`done\` if the deliverable wasn't produced — use \`blocked\` with the real reason).

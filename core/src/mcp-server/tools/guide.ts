...
\`ccr_restart(session_id)\` kills the owner and resumes it fresh (same id, same history, fresh MCP tools). Every result carries \`screen\` — the session's visible pane, plus \`tmuxSession\` — because the two states that matter are indistinguishable any other way.

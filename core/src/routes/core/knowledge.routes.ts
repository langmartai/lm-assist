...
export function createKnowledgeRoutes(_ctx: RouteContext): RouteHandler[] {
  ...
  return [
    ...
    // Delegates to KnowledgeLlmReviewer singleton (concurrency-safe, tracks cost + history).
    // Body: { model?: 'opus'|'sonnet'|'haiku', limit?: number, project?: string, forceReview?: boolean }
    {
      method: 'POST',
      pattern: /^\/knowledge\/review\/llm$/,
      handler: async (req) => {
        const { getKnowledgeLlmReviewer } = require('../../knowledge/llm-reviewer');
        const reviewer = getKnowledgeLlmReviewer();
        const { model, limit, project, forceReview } = req.body || {};

        try {
          const status = await reviewer.review({ model, limit, project, forceReview, trigger: 'manual' as const });
    ...
    // GET /knowledge/review/llm/status — Live processing state + review coverage stats
    {
      method: 'GET',
      ...
      handler: async (req) => {
        ...
        // Store-level stats (scoped to project if specified)
        ...
        const byRating: Record<string, number> = { good: 0, borderline: 0, bad: 0, unreviewed: unreviewed.length };
        for (const k of reviewed) {
          if (k.reviewRating) byRating[k.reviewRating]++;
        }

        // Merge with live reviewer status
        const liveStatus = reviewer.getStatus();
        ...
        return {
          ...
          data: {
            ...
            ratings: byRating,
            processing: liveStatus.status === 'processing',
            currentModel: liveStatus.model,
            trigger: liveStatus.trigger,
            startedAt: liveStatus.startedAt,
            lastCompletedAt: liveStatus.lastCompletedAt,
          },
        };
      },
    },

    // GET /knowledge/review/llm/history — Review batch history with cost data
    {
      method: 'GET',
      pattern: /^\/knowledge\/review\/llm\/history$/,
      handler: async (req) => {
        const { getKnowledgeLlmReviewer } = require('../../knowledge/llm-reviewer');
        const reviewer = getKnowledgeLlmReviewer();

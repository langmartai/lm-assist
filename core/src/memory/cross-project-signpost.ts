...
export function renderSignpost(_self: ProjectRef, others: ProjectRef[]): string {
  const lines: string[] = [
    ...
    '',
    '- `memory_projects` — list every project that has curated memory (+ its slug).',
    '- `search_memory` — search your memory across ALL projects (each hit tagged with its project).',
    '- `memory_map` — what memory exists per project; `memory_record` — read a specific entry.',
    '- `memory_cross_host` / `memory_import_candidates` — portable knowledge mirrored from / newer on other hosts.',
    '',

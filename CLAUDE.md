...
| [YouTube connector](docs/youtube-connector.md) | `youtube_*` tools · `core/src/youtube/` · `/youtube/*` routes · transcript · channel videos · `TRANSCRIPT_EMPTY` · Chrome port 9225 | The DATA path is a node-side fetch parsing the JSON YouTube embeds — the CDP browser is only the authenticated FALLBACK (a measured LAN blocked every Chrome from youtube.com while curl passed). The web caption endpoint answers an **empty HTTP 200** without a browser token — empty ≠ no captions; the innertube ANDROID fallback gets the real track. |

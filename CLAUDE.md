...
| [bundled ext plugins](docs/mcp-plugins-bundled.md) | `core/data/mcp-plugins/` · `bundled.json` · `bundledOptOut` · `LM_BUNDLED_PLUGINS` · `core.sh plugins` · "ship a plugin with lm-assist" · a plugin missing on a fresh node · editing a vendored plugin payload | 🔴 `core/data/mcp-plugins/*` are MIRRORS of repos this one does not own, and an upstream may be PRIVATE while lm-assist is PUBLIC + AGPL. Never edit a mirror — fix it upstream and re-vendor with `--from`; the script refuses a locally-edited mirror. Vendoring PUBLISHES the payload under AGPL, so confirm the owner intends that, and NEVER let an upstream's repo URL/owner/author-email reach `bundled.json`, a payload manifest, a route response or these docs — provenance is a checksum, not an identity. The index is NOT optional, and `payloadChecksum()` EXCLUDES the manifest so the digest is pinned separately. Package trust only means "byte-for-byte what this build shipped" — NOT a defence against a tampered tarball. |
...
| [data export / import + takeover](docs/data-export-import.md) | `data_export` · `data_import` · `/data/bundles` · `core/src/data/bundle/` · `.lmbundle.gz` · lmbundle · export / import / back up / restore lm-assist data · "is the backlog safe?" · takeover · `supersedes` · `data-snapshot` · `REPLICA_READ_ONLY` · `OWNER_ONLINE` | **Replication is NOT backup**: replicas take a bad write within one reconcile and are never re-served — the only restore points are bundles, and the `data-snapshot` job ships DISABLED. Import a dataset on its ORIGIN (or ONE intended owner): a node without the dataset CREATES a second owner, and the owner-online guard only knows the bundle's recorded owner. `apply` writes nothing without `confirm:true`. |
...
| [UI pane deploy](docs/ui-panes-deploy.md) | `ui-apps/` · `~/.lmui/apps` · `assets/lmui.js` · pane shim · `local-tier/` · `/viewtoken/remint` · "deploy a pane" · `core.sh panes` · STALE PANE SHIMS | A Core build does **not** deploy panes — the shim rides a human's `cp`, the server rides the build, and a mismatch only bites when a pane's 15-min token expires. Run `./core.sh panes sync`. A new request field must be OPTIONAL: 4 deployed panes inline the shim into a bundled `app.js` and NO copy can fix them. `scope: assist-web` panes are GATEWAY-hosted (files on SG, serve with no node up) — the node-side sync deliberately skips them; asserting one from a node re-ties it to that node. |
...
| [VM management](docs/vm-management.md) | `vm_*` tools · `core/src/vm/` · `/vm/*` routes · Hyper-V · KVM · virsh · `VM_NOT_MANAGED` · `UNSAFE_PATH` · create a VM · elevation | The input CHARSET regexes are the security boundary for elevated commands — widening one widens what reaches a privileged shell. BOTH backends are e2e-verified (Hyper-V 107, KVM 117); `direct` privilege proves **virsh**, not the filesystem — helpers fall back to `sudo -n` on EACCES. |
| [container management](docs/container-management.md) | `container_*` tools · `core/src/container/` · `/container/*` routes · docker · Docker Desktop · Docker Engine · `DOCKER_UNAVAILABLE` · `CONTAINER_NOT_MANAGED` · run a container · bind mount · image | Bind mounts are REFUSED until a node sets `volumeRoots` — a mount hands a host directory to a process that is usually root. `stop`/`restart` are gated on the managed label too, not just delete: this fleet runs REAL services in containers. Availability means a `docker version` **Server** block — the CLI alone proves nothing (107 has the CLI and no daemon). |
